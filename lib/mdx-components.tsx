...
export const mdxComponents: Record<string, React.ComponentType<any>> = {
  ...
  p: ({ className, ...props }) => (
    <p
      className={clsx("leading-7 [&:not(:first-child)]:mt-6", className as string)}
      {...props}
    />
  ),
  ul: ({ className, ...props }) => (
    <ul className={clsx("my-6 ml-6 list-disc", className as string)} {...props} />
  ),
  ol: ({ className, ...props }) => (
    <ol className={clsx("my-6 ml-6 list-decimal", className as string)} {...props} />
  ),
  li: ({ className, ...props }) => (
    <li className={clsx("mt-2", className as string)} {...props} />
  ),
  ...
  img: ({ className, alt = "", ...props }) => (
    <img
      className={clsx("rounded-md border border-zinc-200", className as string)}
      alt={String(alt)}
      loading="lazy"
      decoding="async"
  ...
  table: ({ className, ...props }) => (
    <div className="w-full my-6 overflow-y-auto">
      <table className={clsx("w-full", className as string)} {...props} />
    </div>
  ),
  tr: ({ className, ...props }) => (
    <tr
